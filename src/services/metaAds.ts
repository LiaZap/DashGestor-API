...
export async function fetchMetaAdsets(campaignId: string) {
...
export async function fetchMetaAds(adsetId: string) {
...
export async function fetchMetaDemographics(period: string) {
  const accountId = getMetaAdAccountId();
  const { since, until } = periodToDates(period);
